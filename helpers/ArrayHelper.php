 ...
 * application.
 * This helper has been ported from yii 2.0. So licence and authoring stays as it was.
...
class ArrayHelper
{
	 ...
	 * @throws \CException if the $descending or $sortFlag parameters do not have
	...
	public static function multisort(
		...
		if (is_scalar($descending)) {
			...
			throw new \CException('The length of $descending parameter must be the same as that of $keys.');
		}
		...
		if (is_scalar($caseSensitive)) {
			...
			throw new \CException('The length of $caseSensitive parameter must be the same as that of $keys.');
		}
		$args = array();
