 ...
 */
namespace wheels\actions;

/**
 * wheels\actions\Toggle works in conjunction with [[ToggleButton]] widget in order to ease the task of the developer
 * to update the the attribute displayed on the grid. Just attach to the controller you wish to make the calls to.
 *
 * @author Antonio Ramirez <amigo.cobos@gmail.com>
 * @package wheels\widgets
 * @since 1.0
 */
class Toggle extends \CAction
{
	/**
	...
	public function run($id, $attribute)
	{
		if (\Yii::app()->getRequest()->isPostRequest) {
			...
			throw new \CHttpException(\Yii::t('wheels', 'Invalid request'));
		}
	}
