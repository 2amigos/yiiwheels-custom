...
/**
 * wheels\actions\ToggleColumn renders a toggle button to toggle values of a column
 *
 * @author Antonio Ramirez <amigo.cobos@gmail.com>
 * @package wheels\widgets
 * @since 1.0
 */
class WhToggleColumn extends \CDataColumn
{
	/**
	...
	protected function initButton()
	{
		if ($this->checkedButtonLabel === null) {
			$this->checkedButtonLabel = \Yii::t('wheels', 'Uncheck');
		}
		if ($this->uncheckedButtonLabel === null) {
			$this->uncheckedButtonLabel = \Yii::t('wheels', 'Check');
		}
		if ($this->emptyButtonLabel === null) {
			$this->emptyButtonLabel = \Yii::t('wheels', 'Not set');
		}
...
function() {
	...
	{
		$fn = \CJavaScript::encode(ArrayHelperremove($this->toggleOptions, 'click'));

		$class = preg_replace('/\s+/', '.', $this->toggleOptions['htmlOptions']['class']);
		$js = "$(document).on('click','#{$this->grid->id} a.{$class}',$fn);";

		\Yii::app()->getClientScript()->registerScript(md5($js), $js);
	}
